...
final class DashboardViewModel: ObservableObject {
    ...
    private func triggerNotifications() {
        ...
        let rules = recurringExpenses.map(\.rule) + recurringIncome.map(\.rule)
        NotificationManager.shared.scheduleRecurringDueReminders(rules: rules)
    }
    ...
    private func isPendingThisMonth(_ rule: RecurringRule) -> Bool {
        guard let next = RecurringScheduleHelper.nextExecutionDate(for: rule) else { return false }
